...
VENDOR_DIR="$REPO_ROOT/.spf-core/vendor"
OVERLAYS_DIR="$REPO_ROOT/.spf-core/overlays"
...
# 2b. Overlay Local Hooks
if [ -d "$REPO_ROOT/.spf-core/src/hooks" ]; then
    echo "  [OVERLAY] Copying local hooks from src/hooks/..."
    cp -r "$REPO_ROOT/.spf-core/src/hooks/"* "$HOOKS_DIR/"
fi
...
# 5. Consolidate Skills (Rename to SPF Brand)
echo "5. Consolidating skills to SPF brand..."
cd "$SKILLS_DIR"
...
if [ -d "$REPO_ROOT/.spf-core/src/skill-templates" ]; then
    echo "  [TEMPLATES] Injecting stack templates into spf-write-plan..."
    mkdir -p spf-write-plan/templates
    cp -r "$REPO_ROOT/.spf-core/src/skill-templates/"* spf-write-plan/templates/
fi

# spf-exec-plan (Rename from executing-plans)
mv executing-plans spf-exec-plan
sed -i 's/^name: executing-plans/name: spf-exec-plan/' spf-exec-plan/SKILL.md

# Cleanup old shorthand symlinks if they exist
